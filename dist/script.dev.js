...
var contactsContainer = document.querySelector('.contacts-list'); // let phoneNoRegEx = /^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/;

var digitRegEx = /^[0-9]/;
var alphabetLettersRegEx = /^[a-zA-Z\s]+$/;
var personContacts = [];
console.log(personContacts); // object constructor function
...
submitBtn.addEventListener('click', function () {
  ...
  addressORremarks = document.getElementById('addressORremarks').value; // Step 2 : validation on the inputs
  // inputFieldValidation(); //🔖📗problem any alert runs means validation fails still its next line are running.
  // Stop execution if validation fails

  if (!inputFieldValidation()) {
    //?📗🔖 if(!true) means false and it means condtion is false means now outside aor next code(function , line) will run. so only when true we got from this function.
    return false; // return ; // next line dont run or Stop further code execution 
  } // These will only run if validation passes


  clearInputFields(); // These will only run if validation passes

  console.log(userName);
  console.log(phone);
  console.log(addressORremarks); // Step 3 : create the objects from inputs
  // create objects from user inputs or // These will only run if validation passes

  var contact = new Contacts(userName, phone, addressORremarks); //   console.log("contact",contact);
...
var inputFieldValidation = function inputFieldValidation() {
  ...
  } else if (!alphabetLettersRegEx.test(userName)) {
    //📗🔖  only and only allow alphabet characters and spaces for names validation. other than pattern thing then give false. means not fpound por not match
  ...
  if (phone === "") {
    ...
    return false; // or  //return;    //---> 🔖When you use just return without its value, the function returns undefined by default.In JavaScript, undefined is a falsy value. // Stop further execution
  } else if (!digitRegEx.test(phone)) {
    // Checks if the input contains anything other than digits then it gives true
...
var clearInputFields = function clearInputFields() {
  //  used 2 time. fuction reusablity utilized. 1.clear after add button click  2. clear button press
  var userName = document.getElementById('name');
  var phone = document.getElementById('contact-num');
  var addressORremarks = document.getElementById('addressORremarks');
  userName.value = " ";
  phone.value = " ";
